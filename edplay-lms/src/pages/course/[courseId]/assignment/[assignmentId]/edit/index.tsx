...
export default function EditAssignmentPage() {
  ...
  const { data: assignmentData, isLoading } = trpc.assignment.getById.useQuery({
    id: assignmentId,
  });
  ...
  }, [assignmentData, initializeContents]);
  ...
  if (!assignmentId || isNaN(assignmentId))
    return <p>Assignment tidak valid.</p>;
  if (isLoading || !assignmentData) return <p>Loading...</p>;
    ...
    <div className="min-h-screen bg-[#FDF8F4] px-4 py-6">
      <div className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-md space-y-6">
        ...
        {contents.map((c) => (
          <div
            key={c.id}
            className="border p-4 rounded-md bg-[#FDFDFD] shadow-sm space-y-2"
          >
            <Input
              value={c.contentTitle}
              onChange={(e) =>
                updateContent(c.id, { contentTitle: e.target.value })
              }
            ...
            {(c.contentType === 'LINK' || c.contentType === 'VIDEO') && (
              <Input
                value={c.contentData}
                placeholder={
                  c.contentType === 'LINK' ? 'https://...' : 'YouTube Link'
                }
                onChange={(e) =>
                  updateContent(c.id, { contentData: e.target.value })
                }
              />
            )}
            {c.contentType === 'FILE' && (
